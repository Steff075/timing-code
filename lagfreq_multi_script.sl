...
%variable obs_id = ["0148010301","0506200401","0506200501","0511580101","0511580201","0511580301","0511580401","0653510301","0653510401","0653510501","0653510601"]; % Hi flux

%variable obs_id = ["0110890201","0506200301"]; %Medium flux

variable obs_id = ["0554710801","0506200201"]; %Lo flux
...
%open_plot("1H0707-495_new_hiflux_lagfreq.gif/gif"); % create a gif image
xlog;
ylin;
popt.res = 1;
ylabel("Time Lag (s)");
xlabel("Frequency (Hz)");
title("1H0707-495 lo-flux lag-frequency >8e-5 Hz"); 
connect_points(0);
charsize(1.2); % good character size
nice_width; %good line ratios
plotxy(x, dxm, dxp, lag, delta_lag, delta_lag; dcol=1, decol=4, dsym=6, mcol=2,xrange=[5e-5,1e-2],yrange=[-200,400]);
...
fp = fopen("time_lags_loflux.csv","w");% Create the file
for (i=0; i<length(x); i++) {
...
% Print lag freq values to csv file
fp = fopen("time_lags_loflux2.csv","w");% Create the file
for (i=0; i<length(x); i++) {
  fprintf(fp,"%e,%e,%e,%e\n", x[i], delta_freq[i], lag[i], delta_lag[i]);
}
fclose(fp);
